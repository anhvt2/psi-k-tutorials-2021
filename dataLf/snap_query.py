...
def random_wait(lower_bound, upper_bound):
  ...
  print('Waiting for %.4f seconds...' % (wait_time))
...
# look up result
inTable = np.loadtxt('input.lf.dat')
outTable = np.loadtxt('output.lf.dat')

# specify constraint: c must be negative
tol = 1e-1
c = np.sum(i[:2]) - 54 - tol
print('c = %d' % c)
print('i = [%d, %d, %d]' % (i[0], i[1], i[2]))
if c < 0:
...
random_wait(lower_bound=2, upper_bound=10)
